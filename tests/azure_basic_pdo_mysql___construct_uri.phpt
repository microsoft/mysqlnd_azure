...
--SKIPIF--
<?php
require_once(__DIR__ . DIRECTORY_SEPARATOR . 'skipif_azure_basic.inc');
require_once(__DIR__ . DIRECTORY_SEPARATOR . 'azure_basic_mysql_pdo_test.inc');
MySQLPDOTest::skip();
?>
--FILE--
<?php
	...
	try {
		...
		if ($tmp = MySQLPDOTest::getTempDir()) {
			...
			if ($fp = @fopen($file, 'w')) {
				fwrite($fp, sprintf('mysql:host=%s;dbname=letshopeinvalid;',PDO_MYSQL_TEST_HOST));
				fclose($fp);
				clearstatcache();
				assert(file_exists($file));
				try {
